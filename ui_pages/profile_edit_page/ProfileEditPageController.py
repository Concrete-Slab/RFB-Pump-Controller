...
from microcontroller import MicrocontrollerProfile, AutoGeneratedProfile, PinDefs, save_profile, read_profile, CodeGenerationException, maybe_generate_code
...
class NewProfilePageController(UIController):
    ...
    def __init__(self, root, debug=False):
        ...
        self.add_listener(MEvents.SaveAutoProfile,self.__save_profile)
        self.add_listener(MEvents.SaveManualProfile,self.__save_profile)
    ...
    def __save_profile(self,event: MEvents.SaveAutoProfile|MEvents.SaveManualProfile):
    ...
    def _generate_code(self,event: MEvents.GenerateCode):
        try:
            if isinstance(self.__profile,AutoGeneratedProfile):
                ...
                compareto = None
            pwd = maybe_generate_code(event.name,[PinDefs.from_tuple(tp)for tp in event.pin_assignments],compare_to=compareto)
